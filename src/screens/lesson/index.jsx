...
const LessonScreen = ({ route, navigation }) => {
  ...
  const generateRandomOptions = () => {
    ...
    for (let i = 0; i < 4; i++) {
      if (i === correctIndex) {
        ...
        options.push(
          isQuizMode
            ? vocabulary[currentWordIndex].eng
            : vocabulary[currentWordIndex].vie
        );
      } else {
        // Add random option
        let randomIndex;
        ...
        options.push(
          isQuizMode ? vocabulary[randomIndex].eng : vocabulary[randomIndex].vie
        );
  ...
  const handleCheckAnswer = () => {
    ...
    if (
      (isQuizMode &&
        options[selectedOptionIndex] === vocabulary[currentWordIndex].eng) ||
      (!isQuizMode &&
        options[selectedOptionIndex] === vocabulary[currentWordIndex].vie)
    ) {
      setScore(score + 1);
    }
  };
  ...
  return (
    <SafeAreaView className="h-full flex">
      ...
      <View className="flex-row items-center justify-between mb-4">
        ...
        {!isQuizMode ? (
          <Pressable
            className="flex-row flex-1 items-center gap-x-2"
            onPress={() => handlePlayAudio(currentWord.audio)}
          >
            <Ionicons name="md-volume-medium" size={30} color="black" />
            <Text className="text-xl">{currentWord.eng}</Text>
          </Pressable>
        ) : (
          <Text className="text-xl mx-6">{currentWord.quiz}</Text>
        )}
        {/* Score */}
      ...
      <View className="gap-y-4 flex-1">
        ...
        {options.map((option, index) => (
          ...
          >
            <Text className="text-xl text-black">{option}</Text>
          </TouchableOpacity>
        ))}
      </View>
      ...
      <View className="flex-row">
        <TouchableOpacity
          ...
          className={`flex-1 py-6 shadow ${
            selectedOptionIndex === null || isAnswerChecked
              ? "bg-secondaryLight"
              : "bg-secondary"
          }`}
        >
        ...
        {isQuizMode && currentWordIndex === vocabulary.length - 1 ? (
          <TouchableOpacity
            ...
            className={`flex-1 py-6 shadow ${
              isAnswerChecked ? "bg-primary" : "bg-primaryLight"
            }`}
          >
            <Text className="text-neutral text-center font-bold text-2xl">
              Hoàn thành
